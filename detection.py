...
def detectVehicles(frame, ALLOWED_CLASSES):
    ...
    if mask == None or routes == None or os.getenv('UPDATE_POLYGONS') == 'True':
        mask, routes = getPolygons()
        ...
        if not track.counted:
            ...
            if len(routes) > 0: 
                for name, poly in routes.items():                                    
                    ...
                    if result >= 0 and track.origin is not None and name != track.origin:
                        ...
                        # Add vehicle data to Dataframe
                        data = data.append({'VehicleID': vID, 'Class': track.get_class(), 'IntersectionOrigin': track.origin, 'IntersectionExit': track.exit, 'Timestamp': datetime.now().strftime(
                        ...
                        print('{0} #{1} came from {2} and went to {3}. Pos. points: {4}'.format(
                            track.get_class().capitalize(), str(track.track_id), track.origin, track.exit, track.points))
                        break
            ...
            # Not using detection areas          
                ...
                if vID == None:
                    ...
                    data = data.append({'VehicleID': vID, 'Class': track.get_class(), 'IntersectionOrigin': 'None', 'IntersectionExit': 'None', 'Timestamp': datetime.now().strftime(
                                "%Y-%m-%d %H:%M:%S")}, ignore_index=True)                    
